...
folder are mapped to the application's root namespace, 'BoardGameCreator'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "BoardGameCreator"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'BoardGameCreator' in the same way
as `"BoardGameCreator/sass/src"`.
